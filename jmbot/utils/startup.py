...
LOGS = logging.getLogger("اعداد سورس صوفي")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await jmbot(UnblockRequest("@x_xxi"))
        await jmbot(UnblockRequest("@SoFeThonBot"))
        await jmbot(
            UpdateNotifySettingsRequest(
                peer="t.me/SoFeThonBot",
                settings=InputPeerNotifySettings(mute_until=2**31 - 1),
            )
        )
        await jmbot.edit_folder("@SoFeThonBot", folder=1) 
        channel_usernames = [
            "SoFeThon",
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await jmbot.send_message("@SoFeThonBot", "/start")
            await asyncio.sleep(1)
            await jmbot.send_message(
                "@SoFeThonBot",
                "تم بنجاح تشغيل سورس صوفي عزيزي المستخدم هذا البوت سيتم تشغيله قريبا بعد اكماله",
            )
            await asyncio.sleep(1)
        ...
        try:
            if BOTLOG:
                await jmbot.tgbot.send_file(
                    BOTLOG_CHATID,
                    "https://graph.org/file/4c9492ca51f0483ab953a.jpg",
                    caption="**شكرا لتنصيبك سورسي**\n • هنا بعض الملاحظات التي يجب ان تعرفها عن استخدامك لسورس صوفي.",
        ...
        try:
            if BOTLOG:
                await jmbot.tgbot.send_message(
                    BOTLOG_CHATID,
                    "**لقد تم بنجاح تنصيب سورس صوفي **\n➖➖➖➖➖➖➖➖➖➖\n**السورس**: @SoFeThon\n**المطور**: @x_xxi\n➖➖➖➖➖➖➖➖➖➖\n",
                    buttons=[
                        (Button.url("المطور", "https://t.me/x_xxi"),)
                    ],
                )
        except Exception as e:
            LOGS.error(e)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كروب بوت صوفي", jmbot, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jmbot.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "مجموعة تخزين صوفي", jmbot, Config.TG_BOT_USERNAME, descript, photobt
        )
